...
function toPt(px,scale){
   var x = px[0]-canvas.width/2;
   var y = px[1]-canvas.height/2;
   return [x/scale,y/scale*-1]; 
}
 
function toPx(pt,scale){
   var x = pt[0]*scale;
   var y = pt[1]*scale*-1;
...
function itterate(pt){
  return [[Math.pow(pt[0][0],2)-Math.pow(pt[0][1],2)+pt[1][0],2*pt[0][0]*pt[0][1]+pt[1][1]],pt[1],pt[2],pt[3]];
}
 
function initSet(scale,C){
   var pts = [];
   for(var y=0; y<(scale)+1; y++){
       for(var x=0; x<(scale)+1; x++){
           var pt = toPt([x,y],scale/4);
           if(checkBounded(pt)){
               pts.push([pt,C,pt,[x,y]]);
...
/*
function completeIteration(set,scale,colors,iterationNum){
   var newSet = [];
   var escaped = [];
   for(var s in set){
       var output = itterate(set[s]);
       if(!checkBounded(output[0])){
           var px = toPx(output[2],scale/4);
           ctx1.fillStyle = colors[iterationNum%colors.length];
           ctx1.beginPath();
           ctx1.arc(px[0],px[1],1,0,Math.PI*2);
           ctx1.fill();
       }
       else{
           newSet.push(output);
...
function colorInSet(set,color){
   for(var i in set){
       ...
       ctx1.beginPath();
       ctx1.arc(set[i][3][0],set[i][3][1],1,0,Math.PI*2);
       ctx1.fill();
   }
}
...
function completeIteration(set,colors,iterationNum){
   ...
   for(var s in set){
       var output = itterate(set[s]);
       if(!checkBounded(output[0])){
           escaped.push([output[3],colors[iterationNum%colors.length]]);
       }
       else{
           newSet.push(output);
       }
...
function zoom(scale,iterations,colors,inSetColor,C){
   var drag = false;
   ...
   addEventListener('mouseup',function(evt){
       ...
       if(startPt[0] >= 0 && startPt[0]<=scale && startPt[1] >= 0 && startPt[1]<=scale && mouse.x >= 0 && mouse.x <=scale && mouse.y >=0 && mouse.y <= scale){
           ...
           zoomDraw(pt,scale,iterations,pastZoomInfo,colors,inSetColor,C);
...
function zoomDraw(startPt,origonalScale,iterations,pastZoomInfo,colors,inSetColor,C){
   ctx1.clearRect(0,0,canvas1.width,canvas1.height);
 
   var set = zoomSet(origonalScale,startPt,pastZoomInfo,inSetColor,C);
   var escaped = [];
...
function zoomSet(initialScale,startPt,pastZoomInfo,inSetColor,C){
   var pts = [];
 
   for(var x = 0; x<(initialScale); x++){
       for(var y=0; y<(initialScale); y++){
           var pt = [[(pastZoomInfo[0]*(x/(initialScale/4)))+startPt[0],-1*((pastZoomInfo[0]*(y/(initialScale/4))-startPt[1]))],C,[(pastZoomInfo[0]*(x/(initialScale/4)))+startPt[0],-1*((pastZoomInfo[0]*(y/(initialScale/4)))-startPt[1])],[x,y]];
...
function init(scale,iterations,C){
   var colors = [];
   ...
   //-------GRAYSCALE---------
   for(var i =256; i>0; i-=2){
       colors.push('rgb('+i+','+i+','+i+')')
   }
 
   var set = initSet(scale,C);
   var escaped = [];
   for(var i=0; i<iterations; i++){
       pastEscaped = escaped;
       sets = completeIteration(set,colors,i);
   ...
   zoom(scale,iterations,colors,inSetColor,C);
}
 
 
init(400,200,[-0.8,0.156]);
